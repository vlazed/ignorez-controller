...
function TOOL:Think()
    if CLIENT then
        if self:GetControlledEntity() == lastSelectedEntity and lastSelectedEntityValid == IsValid(lastSelectedEntity) then return end
...
local function materialList(panel, materials)
    local matList = vgui.Create("DListView", cPanel)
...
local function getMaterialProps(entity, materialName)
    for _, matInfo in ipairs(entity.izc_materials) do
        if matInfo.name == materialName then return matInfo.props end
    end
end
...
function TOOL.BuildCPanel(panel, entity)
    ...
    controlMaterial.OnChange = function(_, checked)
        if #matList:GetSelected() == 0 then return end
        if checked then
    ...
    maxLookAngle.OnValueChanged = function(_, newValue)
        if settingProps then return end
        updateMaterialProps()
    end
